...
uint16_t cpu_fetch(void)
{
	...
	uint16_t ret = (g_machine.memory[g_machine.cpu.pc] << 8) |
				   (g_machine.memory[g_machine.cpu.pc + 1]);
	g_machine.cpu.pc += 2;
	return ret;
}
...
void cpu_execute(uint16_t instruction)
{
	...
	case 0x08: {
		...
		case 0x04: {
			g_machine.cpu.v[0x0F] =
				((uint16_t)(g_machine.cpu.v[x] + g_machine.cpu.v[y]) > 255) ?
					1 :
					0;
			g_machine.cpu.v[x] += g_machine.cpu.v[y];
			break;
		...
		case 0x05: {
			g_machine.cpu.v[0x0F] =
				(g_machine.cpu.v[y] <= g_machine.cpu.v[x]) ? 1 : 0;
			g_machine.cpu.v[x] -= g_machine.cpu.v[y];
			break;
		}
		case 0x06: {
	...
	case 0x0D: {
		...
		for (uint8_t i = 0; i < n; i++) {
			const uint8_t sprite_data =
				g_machine.memory[g_machine.cpu.index + i];
			ren_x = ox;

			for (int8_t j = 7; j >= 0; j--) {
				uint8_t *pixel =
					&g_machine.framebuffer[ren_y * WINDOW_WIDTH + ren_x];
				...
				*pixel ^= sprite_bit;
				...
				if (++ren_x >= WINDOW_WIDTH)
					break;
			}

			if (++ren_y >= WINDOW_HEIGHT)
				break;
		}
	...
	case 0x0F: {
		...
		case 0x0A: {
			...
			for (uint8_t i = 0; i < sizeof(g_machine.keypad) && key == 0xFF;
				 i++) {
		...
		case 0x29: {
			g_machine.cpu.index =
				g_machine.memory[g_machine.cpu.index + g_machine.cpu.v[x]];
			break;
		}
		case 0x33: {
