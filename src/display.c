...
{
	g_machine.win = SDL_CreateWindow(
		"CHIP-8 Emulator", 100, 100, WINDOW_WIDTH * WINDOW_SCALE_FACTOR,
		WINDOW_HEIGHT * WINDOW_SCALE_FACTOR, SDL_WINDOW_SHOWN);
	if (g_machine.win == NULL) {
		fprintf(stderr, "SDL_CreateWindow Error: %s\n", SDL_GetError());
		return EXIT_FAILURE;
	}

	g_machine.ren = SDL_CreateRenderer(g_machine.win, -1,
...
void display_update(void)
{
	...
	for (uint32_t i = 0; i < sizeof(g_machine.framebuffer); i++) {
		...
		if (g_machine.framebuffer[i] == 1) {
			SDL_SetRenderDrawColor(g_machine.ren, 255, 255, 255,
								   SDL_ALPHA_OPAQUE);
